...
def _build_mlp(input_dim: int, output_dim: int, hidden_dims) -> nn.Module:
    hidden_dims = list(hidden_dims or [])
    layers = []
    last_dim = input_dim
    for dim in hidden_dims:
        layers.append(nn.Linear(last_dim, int(dim)))
...
class DiffusionUnetHybridImagePolicy(BaseImagePolicy):
    def __init__(self,
        ...
        self.persistent_action_source = str(
            persistent_action.pop('source', 'initial_obs_estimator')).lower()
        persistent_action_estimator_hidden_dims = persistent_action.pop(
            'estimator_hidden_dims', [256, 256])
        self.persistent_action_loss_weight = float(
            persistent_action.pop('loss_weight', 1.0))
        ...
        if self.persistent_action_source not in {'initial_obs_estimator'}:
            raise ValueError(
                f'Unsupported persistent_action source: {self.persistent_action_source}')
        self.persistent_action_dim = len(self.persistent_action_indices)
        self.persistent_action_estimator = None
        if self.persistent_action_enabled:
            self.persistent_action_estimator = _build_mlp(
                input_dim=obs_feature_dim,
                output_dim=self.persistent_action_dim,
                hidden_dims=persistent_action_estimator_hidden_dims)
        ...
        if self.persistent_action_enabled:
            if global_cond_dim is None:
                global_cond_dim = 0
            global_cond_dim += self.persistent_action_dim
        ...
        self._persistent_action_initialized = None
        ...
        if self.persistent_action_enabled:
            ...
            print("Persistent estimator params: %e" % sum(
                p.numel() for p in self.persistent_action_estimator.parameters()))
    ...
    def reset(self, mask: Optional[torch.Tensor] = None):
        ...
        if self.persistent_action_enabled and self._persistent_action_value is not None:
            ...
            self._persistent_action_initialized = self._persistent_action_initialized.clone()
            self._persistent_action_initialized[applied_mask] = False
    ...
    def _estimate_persistent_action_from_initial_obs(
            self, obs_dict: Dict[str, torch.Tensor]) -> Optional[torch.Tensor]:
        if not self.persistent_action_enabled:
            return None
        nobs = self.normalizer.normalize(obs_dict)
        first_nobs = dict_apply(
            nobs,
            lambda x: x[:, :1, ...].reshape(-1, *x.shape[2:]))
        initial_obs_feature = self.obs_encoder(first_nobs)
        return self.persistent_action_estimator(initial_obs_feature)

    def _append_persistent_action_condition(
            self,
            global_cond: Optional[torch.Tensor],
            persistent_action_value: Optional[torch.Tensor]) -> Optional[torch.Tensor]:
        if persistent_action_value is None:
            return global_cond
        normalized_persistent = self._normalize_persistent_action_value(
            persistent_action_value)
        if global_cond is None:
            return normalized_persistent
        return torch.cat([global_cond, normalized_persistent], dim=-1)

    def _get_or_initialize_online_persistent_action(
            self, obs_dict: Dict[str, torch.Tensor]) -> Optional[torch.Tensor]:
        if not self.persistent_action_enabled:
            return None
        ...
        if needs_reset:
            self._persistent_action_value = torch.zeros(
                (batch_size, self.persistent_action_dim),
                device=device,
                dtype=dtype)
            self._persistent_action_initialized = torch.zeros(
                (batch_size,),
                device=device,
                dtype=torch.bool)

        uninitialized_mask = ~self._persistent_action_initialized
        if uninitialized_mask.any():
            estimated_value = self._estimate_persistent_action_from_initial_obs(obs_dict)
            self._persistent_action_value = self._persistent_action_value.clone()
            self._persistent_action_value[uninitialized_mask] = estimated_value[uninitialized_mask]
            self._persistent_action_initialized = self._persistent_action_initialized.clone()
            self._persistent_action_initialized[uninitialized_mask] = True

        return self._persistent_action_value

    def _get_initial_obs_from_batch(
            self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        if 'initial_obs' not in batch:
            raise RuntimeError(
                "Missing `initial_obs` in batch. Enable task.dataset.return_initial_obs "
                "when using persistent_action.source=initial_obs_estimator.")
        return batch['initial_obs']
    ...
    def _update_history_from_action_chunk(
            self,
    ...
    def _predict_action_internal(self,
            ...
            update_history: bool,
            history_update_steps: Optional[int] = None,
        ...
        global_cond = self._append_persistent_action_condition(
            global_cond,
            persistent_action_value)
        ...
        if update_history and self.action_history_enabled:
            self._update_history_from_action_chunk(
                naction,
                max_update_steps=history_update_steps)
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        persistent_action_value = self._get_or_initialize_online_persistent_action(obs_dict)
        if self.action_history_enabled:
            value = next(iter(obs_dict.values()))
    ...
    def predict_action_from_batch(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        initial_obs = self._get_initial_obs_from_batch(batch)
        persistent_action_value = self._estimate_persistent_action_from_initial_obs(initial_obs)
        return self._predict_action_internal(
            obs_dict=batch['obs'],
            history_feature=history_feature,
    ...
    def compute_loss(self, batch):
        ...
        history_feature = self._encode_action_history_from_batch(batch)
        initial_obs = self._get_initial_obs_from_batch(batch)
        predicted_persistent_action_value = self._estimate_persistent_action_from_initial_obs(
            initial_obs)
        ...
        global_cond = self._append_persistent_action_condition(
            global_cond,
            predicted_persistent_action_value)
        ...
        loss = loss.mean()
        if predicted_persistent_action_value is not None and persistent_action_value is not None:
            persistent_loss = F.mse_loss(
                predicted_persistent_action_value,
                persistent_action_value)
            loss = loss + self.persistent_action_loss_weight * persistent_loss
        return loss
